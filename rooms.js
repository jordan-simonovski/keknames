...
function broadcastState(io, room) {
  ...
  for (const p of room.players) {
    ...
    io.to(p.socketId).emit('game-state', { ...view, playerAvatars: avatarMap, players: roster, you: { id: p.id, name: p.name, team: p.team, role: p.role, isHost: p.id === room.host, isSolo: solo, avatarId: p.avatarId } });
  }
}
...
function setupRoomHandlers(io, socket) {
  ...
  socket.on('join-room', ({ code, name }, cb) => {
    ...
    if (room.game) {
      ...
      socket.emit('game-state', { ...view, you: { id: player.id, name: player.name, team: player.team, role: player.role, avatarId: player.avatarId } });
    }
  });
