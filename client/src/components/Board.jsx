...
const AVATAR_SRCS = Array.from({ length: 8 }, (_, i) =>
  `/assets/ui/avatar_${String(i + 1).padStart(2, '0')}.png`
);
preloadImages([...TYPE_SRCS, ...AVATAR_SRCS]);

export default function Board({ gameState, isSpymaster, voteInfo, onCardClick }) {
  const canvasRef = useRef(null);
  const stateRef = useRef({
    cards: [], mode: 'words', cols: 5, rows: 5,
    cardW: 0, cardH: 0, gap: 6,
    hoverIndex: -1,
  ...
  const getCardRect = useCallback((index) => {
    ...
    if (!card.revealed && !anim && s.voteInfo && s.voteInfo.showVotes && !s.voteInfo.isSolo) {
      ...
      if (voters && voters.length > 0) {
        ...
        for (let vi = 0; vi < voters.length; vi++) {
          ...
          ctx.clip();
          if (avImg) { ctx.drawImage(avImg, ax, ay, avatarSize, avatarSize); }
          else { ctx.fillStyle = isMine ? '#f39c12' : '#aaa'; ctx.fill(); }
          ctx.restore();
          ctx.beginPath();
          ctx.arc(ax + avatarSize / 2, ay + avatarSize / 2, avatarSize / 2, 0, Math.PI * 2);
  ...
  useEffect(() => {
    ...
    function handleTouchEnd(e) {
      ...
      if (idx >= 0 && onCardClick) { e.preventDefault(); onCardClick(idx); }
    }
    ...
    function handleMouseLeave() {
      const s = stateRef.current;
      if (s.hoverIndex !== -1) { s.hoverIndex = -1; draw(); }
    }
