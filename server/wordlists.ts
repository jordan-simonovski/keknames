...
const GILMOREGIRLS_EASY = dedupe([
  ...
  'SOOKIE', 'LANE', 'PARIS', 'JESS', 'DEAN',
  'LOGAN', 'KIRK', 'BABETTE', 'TAYLOR', 'MICHEL',
  'COFFEE', 'DINER', 'GAZEBO', 'YALE', 'CHILTON',
  'FRIDAY', 'DINNER', 'INN', 'DRAGONFLY', 'INDEPENDENCE',
  'CONNECTICUT', 'HARTFORD', 'NEWSPAPER', 'BOOKWORM', 'TOWNMEETING',
  'TROUBADOUR', 'CATHOUSE', 'BANGLES', 'MAILMAN', 'BRIDGE',
  'FESTIVAL', 'SNOWMAN', 'POTLUCK', 'MARATHON', 'BASKET',
]);

const GILMOREGIRLS_HARD = dedupe([
  'STARHOLLOW', 'DRAGONFLY', 'INDEPENDENCE', 'CHILTON', 'HAYDEN',
  'HUNTZBERGER', 'GILMORE', 'DANES', 'MARIANO', 'FORESTER',
  'BELLEVILLE', 'DOYOULIKE', 'COPPERBOOK', 'MISPATTY', 'GYPSY',
  'LEMON', 'DIGGER', 'CHRISTOPHER', 'STRAUB', 'TWICKHAM',
  'TANKARD', 'POPTART', 'MALLOMAR', 'BOPPIT', 'BIDDY',
  'LACEY', 'DAVEY', 'DOULA', 'KWAN', 'CINNAMON',
  'YEARBOOK', 'FRANKLIN', 'LADB', 'MARTY', 'COLIN',
  'FINN', 'BRIGADOON', 'TWIRL', 'COPACETIC', 'VINDICATOR',
]);
...
const STRANGERTHINGS_EASY = dedupe([
  ...
  'HOPPER', 'JOYCE', 'NANCY', 'STEVE', 'JONATHAN',
  'MAX', 'BILLY', 'ROBIN', 'EDDIE', 'VECNA',
  'DEMOGORGON', 'MINDFLAYER', 'HAWKINS', 'UPSIDE', 'GATE',
  'WAFFLE', 'BIKE', 'WALKIE', 'ARCADE', 'DUNGEON',
  'DRAGON', 'LABYRINTH', 'NOSEBLEED', 'TELEKINESIS', 'VANISH',
  'FLASHLIGHT', 'CHRISTMAS', 'ALPHABET', 'MONSTER', 'SHADOW',
  'PORTAL', 'WOODS', 'QUARRY', 'PUMPKIN', 'SNOW',
]);

const STRANGERTHINGS_HARD = dedupe([
  'BRENNER', 'OWENS', 'BYERS', 'WHEELER', 'HARRINGTON',
  'SINCLAIR', 'HENDERSON', 'MAYFIELD', 'MUNSON', 'CREEL',
  'DEMOBAT', 'DEMODOGS', 'THESSALHYDRA', 'FLAYER', 'UMBER',
  'NINA', 'RAINBOW', 'MADMAX', 'ZOOMER', 'MIRKWOOD',
  'STARCOURT', 'SCOOPS', 'SURFER', 'HELLFIRE', 'PIGGYBACK',
  'VOID', 'SENSORY', 'DEPRIVE', 'DIMENSION', 'RIFT',
  'SATELLITE', 'CONSPIRACY', 'ELECTRODE', 'SPECIMEN', 'CASSETTE',
  'NEVERENDING', 'RUNAWAY', 'METALLIC', 'MASTEROF', 'PALADIN',
]);

// ---------- Gaming ----------

const GAMING_EASY = dedupe([
  'MARIO', 'ZELDA', 'POKEMON', 'SONIC', 'HALO',
  'MINECRAFT', 'FORTNITE', 'TETRIS', 'PACMAN', 'DONKEY',
  'KIRBY', 'METROID', 'SMASH', 'KART', 'SPLATOON',
  'PLAYSTATION', 'XBOX', 'NINTENDO', 'STEAM', 'CONSOLE',
  'CONTROLLER', 'JOYSTICK', 'HEADSET', 'RESPAWN', 'LOOT',
  'QUEST', 'BOSS', 'LEVEL', 'HEALTH', 'ARMOR',
  'INVENTORY', 'CRAFTING', 'SANDBOX', 'PIXEL', 'RETRO',
  'COOP', 'MULTIPLAYER', 'SPEEDRUN', 'ACHIEVEMENT', 'LEADERBOARD',
]);

const GAMING_HARD = dedupe([
  'FRAMERATE', 'VSYNC', 'RAYTRACING', 'POLYGON', 'SHADER',
  'VOXEL', 'HITBOX', 'IFRAMES', 'COOLDOWN', 'NERF',
  'BUFF', 'META', 'GRIEFING', 'GANKING', 'AGGRO',
  'ROGUELIKE', 'SOULSLIKE', 'METROIDVANIA', 'BULLETHILL', 'SHMUP',
  'JRPG', 'MOBA', 'GACHA', 'PERMADEATH', 'PROCEDURAL',
  'RIGIDBODY', 'NAVMESH', 'OCCLUSION', 'CULLING', 'DEFERRED',
  'TESSELLATION', 'ANISOTROPIC', 'ANTIALIASING', 'EMULATOR', 'ROMHACK',
  'MODDING', 'OVERCLOCK', 'BENCHMARK', 'NETCODE', 'ROLLBACK',
]);

// ---------- Movies ----------

const MOVIES_EASY = dedupe([
  'INCEPTION', 'TITANIC', 'AVATAR', 'JAWS', 'ROCKY',
  'GLADIATOR', 'FROZEN', 'SHREK', 'MATRIX', 'GODFATHER',
  'JURASSIC', 'TERMINATOR', 'ALIEN', 'PREDATOR', 'BATMAN',
  'JOKER', 'AVENGERS', 'IRONMAN', 'SPIDERMAN', 'GANDALF',
  'HOBBIT', 'HOGWARTS', 'LIGHTSABER', 'DEATHSTAR', 'WOOKIEE',
  'NEMO', 'RATATOUILLE', 'COCO', 'ENCANTO', 'MOANA',
  'POPCORN', 'DIRECTOR', 'SEQUEL', 'PREQUEL', 'TRAILER',
  'SCRIPT', 'STUNTMAN', 'PREMIERE', 'BOXOFFICE', 'CINEMA',
]);

const MOVIES_HARD = dedupe([
  'ROSEBUD', 'MACGUFFIN', 'KUBRICK', 'HITCHCOCK', 'TARANTINO',
  'SCORSESE', 'SPIELBERG', 'NOLAN', 'COPPOLA', 'LYNCH',
  'KULESHOV', 'MONTAGE', 'DOLLY', 'STEADICAM', 'GAFFER',
  'FOLEY', 'DIEGETIC', 'DISSOLVE', 'JUMPCUT', 'TRACKING',
  'NOIR', 'NEOREALISM', 'DOGME', 'ARTHOUSE', 'CRITERION',
  'PALME', 'SUNDANCE', 'CANNES', 'AUTEUR', 'CINEMASCOPE',
  'ANAMORPHIC', 'GRADING', 'COLORIST', 'ROTOSCOPE', 'KEYFRAME',
  'GREENSCREEN', 'PRACTICAL', 'PROSTHETIC', 'ANIMATRONIC', 'MINIATURE',
]);

// ---------- Australia ----------

const AUSTRALIA_EASY = dedupe([
  'KANGAROO', 'KOALA', 'PLATYPUS', 'CROCODILE', 'WOMBAT',
  'EMU', 'DINGO', 'COCKATOO', 'KOOKABURRA', 'QUOKKA',
  'BOOMERANG', 'DIDGERIDOO', 'VEGEMITE', 'BARBIE', 'THONGS',
  'ARVO', 'BREKKIE', 'SERVO', 'STUBBY', 'TINNY',
  'OUTBACK', 'REEF', 'ULURU', 'SYDNEY', 'MELBOURNE',
  'SURFING', 'CRICKET', 'FOOTY', 'LIFEGUARD', 'BONDI',
  'MATES', 'STREWTH', 'RECKON', 'CHUNDER', 'SNAG',
  'FAIRY', 'PAVLOVA', 'LAMINGTON', 'TIMTAM', 'MILO',
]);

const AUSTRALIA_HARD = dedupe([
  'BILLABONG', 'BUNYIP', 'BUDGIE', 'YAKKA', 'DRONGO',
  'BOGAN', 'LARRIKIN', 'OCKER', 'GALAH', 'CHOOK',
  'COBBER', 'DAGGY', 'FURPHY', 'RIPPER', 'STOKED',
  'WOOP', 'YABBER', 'YONKS', 'SPRUIK', 'SUNNIES',
  'ESKY', 'UTE', 'SWAG', 'BILLYCART', 'BLUEY',
  'DROPBEAR', 'REDBACK', 'FUNNEL', 'STINGER', 'BLUEBOTTLE',
  'DREAMTIME', 'CORROBOREE', 'COOLAMON', 'WADDY', 'HUMPY',
  'MULGA', 'SPINIFEX', 'MALLEE', 'GIBBER', 'SALTBUSH',
]);
...
const FRIENDS_EASY = dedupe([
  ...
  'PHOEBE', 'GUNTHER', 'JANICE', 'RICHARD', 'MIKE',
  'COFFEE', 'COUCH', 'APARTMENT', 'FOUNTAIN', 'LOBSTER',
  'PIVOT', 'BREAK', 'SANDWICH', 'WEDDING', 'DIVORCE',
  'TWINS', 'BABY', 'PALEONTOLOGY', 'CHEF', 'MASSAGE',
  'GUITAR', 'SMELLY', 'THANKSGIVING', 'TRIVIA', 'LONDON',
  'VEGAS', 'BARBADOS', 'MUSEUM', 'FASHION', 'ACTING',
  'AUDITION', 'ROOMMATE', 'DATING', 'PROPOSAL', 'ADOPTION',
]);

const FRIENDS_HARD = dedupe([
  'CENTRALPARK', 'GELLER', 'BING', 'BUFFAY', 'TRIBIANI',
  'GREEN', 'HANNIGAN', 'URSULA', 'UNAGI', 'MOIST',
  'MISSISSIPPI', 'TRANSPONSTER', 'REGINA', 'PHALANGE', 'BAMBOOZLE',
  'HOLIDAY', 'ARMADILLO', 'ROUTINE', 'PROM', 'VIDEOTAPE',
  'BACKPACKING', 'LEATHER', 'SALMON', 'FREEBALLIN', 'NUBBIN',
  'GLADYS', 'HUGSY', 'CRUSTY', 'CUSHION', 'PRINCESS',
  'CONSUELA', 'CRAP', 'CHANANDLER', 'BONG', 'CHICK',
  'DUCK', 'PORSCHE', 'BARCALOUNGER', 'MISSISSIPPI', 'MOKONDUIT',
]);
...
const SEINFELD_EASY = dedupe([
  ...
  'SOUP', 'PUFFY', 'MARBLE', 'FESTIVUS', 'SHRINKAGE',
  'SPONGE', 'PRETZEL', 'DOUBLE', 'MASTER', 'APARTMENT',
  'STANDUP', 'COMEDY', 'DATING', 'PARENTS', 'SUBWAY',
  'PARKING', 'CONTEST', 'AIRPORT', 'CHINESE', 'MOVIE',
  'DENTIST', 'LIBRARY', 'JACKET', 'CIGAR', 'COFFEE',
  'DINER', 'CEREAL', 'YANKEES', 'BASEBALL', 'SUPERMAN',
  'MAILMAN', 'PHONE', 'BOTTLE', 'WEDDING', 'INVITE',
]);

const SEINFELD_HARD = dedupe([
  'COSTANZA', 'BENES', 'PETERMAN', 'STEINBRENNER', 'PUDDY',
  'POPPIE', 'BABU', 'BANYA', 'WHATLEY', 'BOOKMAN',
  'YADA', 'SERENITY', 'MANSSIERE', 'MANSIERE', 'FESTIVUS',
  'AIRING', 'GRIEVANCE', 'FEATS', 'KAVORKA', 'MULVA',
  'DOLORES', 'REGRASP', 'MIMBO', 'SPONGEWORTHY', 'CLOSETALKER',
  'LOWTALKER', 'HIGHTALKER', 'SIDLER', 'REGIFTER', 'DOUBLEDIP',
  'ANTIDENTITE', 'SHRINKAGE', 'JIMMY', 'JERKSTORE', 'MARINE',
  'PROGNOSIS', 'VANDELAY', 'PENNYPACKER', 'LATEXSALESMAN', 'ARCHITECT',
]);
...
const CATEGORIES: Record<CategoryId, WordCategory> = {
  ...
  strangerthings: { id: 'strangerthings', label: 'Stranger Things',  easy: STRANGERTHINGS_EASY, hard: STRANGERTHINGS_HARD },
